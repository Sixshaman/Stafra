...
void FractalGen::DoComputingPreparations(const std::wstring& initialBoardFile, const std::wstring& clickRuleFile, const std::wstring& restrictionFile)
{
	...
	mStabilityCalculator->PrepareForCalculations(mRenderer->GetDevice(), mRenderer->GetDeviceContext(), initialBoardTex.Get(), clickRuleTex.Get(), restrictionTex.Get());

	mRenderer->SetCurrentClickRule(mStabilityCalculator->GetClickRule());
}
...
void FractalGen::Tick(uint32_t spawnPeriod, const std::wstring& videoFrameFile)
{
	...
	mStabilityCalculator->StabilityNextStep(mRenderer->GetDeviceContext(), spawnPeriod);
