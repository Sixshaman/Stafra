...
public:
	...
	void PrepareForCalculations(ID3D11Device* device, ID3D11DeviceContext* dc, ID3D11Texture2D* initialBoard, ID3D11Texture2D* clickRule, ID3D11Texture2D* restriction);
	void StabilityNextStep(ID3D11DeviceContext* dc, uint32_t spawnPeriod);
	...
	ID3D11ShaderResourceView* GetClickRule()          const;

private:
	void LoadShaderData(ID3D11Device* device);

	void ReinitTextures(ID3D11Device* device, ID3D11Texture2D* initialBoard, ID3D11Texture2D* clickRule, ID3D11Texture2D* restriction);
	...
	void StabilityNextStepClickRuleSpawn(ID3D11DeviceContext* dc, uint32_t spawnPeriod);
	...
	void StabilityNextStepClickRuleSpawnRestricted(ID3D11DeviceContext* dc, uint32_t spawnPeriod);
